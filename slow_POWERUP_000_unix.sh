...
i=1
echo "percival-hl-scan-setpoints -i 00_0_0V0A -f 01_0_VDD_ON_step_${i} -n 1 -d 2000"

while [ $i -le 3 ]; do
	echo "percival-hl-update-monitors"
	read -p "Continue Ramp UP? [y, n]:" input
	if [[ $input == "n" || $input == "N" ]]; then
		echo "Ramp DOWN Voltage supplies to 0 V, 0 A..."
		echo "percival-hl-scan-setpoints -i 01_0_VDD_ON_step_${i} -f -i 00_0_0V0A -n ${i} -d 2000"
		...
		j=${i}
		i=$(( $i + 1 ))
		echo "percival-hl-scan-setpoints -i 01_0_VDD_ON_step_${j}  -f 01_0_VDD_ON_step_${i} -n 1 -d 2000"
	else
